...
:: Collect .bat files in windows-strategies folder, except service.bat
set "count=0"
for %%f in (windows-strategies\*.bat) do (
    set "filename=%%~nxf"
...
:: Display strategies in columns
set "cols=3"
set /a rows=(count + cols - 1) / cols

echo.
set /a idx=1
for /L %%i in (0,1,%rows%-1) do (
    set "line="
    for /L %%j in (0,1,%cols%-1) do (
        set /a pos=%%i + %%j * %rows%
        set /a pos1=pos + 1
        if !pos1! LEQ %count% (
            call set "nm=%%name!pos1!%%"
            call :pad_item !pos1! "!nm!" padded
            set "line=!line!!padded!"
        )
    )
    echo !line!
)
...
:: helper to pad/format a single item into fixed width column
:pad_item
rem %1 = index, %2 = name (may be quoted), returns %3 variable name containing padded line fragment
setlocal EnableDelayedExpansion
set "idx=%~1"
set "nm=%~2"
set "entry=%idx%) %nm%"
...
set "sp=                                   "
set "entry=!entry!!sp!"
set "entry=!entry:~0,%max%!"
endlocal & set "%~3=%entry%"
exit /b
