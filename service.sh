...
manage_strategy() {
  ...
  if [ $is_running -eq 1 ]; then
    ...
    if ! read -rp "Выберите (1 или 2): " autorun_choice; then return; fi
...
_reinstall_strategy() {
  ...
  if [ -x "$OPT_REPO/install_easy.sh" ]; then
    local install_out
    install_out=$(sudo timeout 15s bash -c "yes '' | bash '$OPT_REPO/install_easy.sh'" 2>&1 || true)
    ...
    if ! sudo systemctl is-active --quiet zapret.service 2>/dev/null; then
      ...
      if [ -z "$err_line" ]; then
          err_line=$(sudo systemctl status zapret.service --no-pager 2>/dev/null | grep -iE 'error|ошибка|failed' | tail -n 1 || true)
      fi
...
delete_zapret() {
  clear_screen
  if ! read -rp "Вы уверены, что хотите полностью удалить Zapret и все изменения? (y/N): " confirm; then return; fi
  case "$confirm" in
    y|Y) ;;
...
utilities_menu() {
  while true; do
    ...
    echo "  0) Back"
    echo ""
    if ! read -rp "Выберите опцию: " uchoice; then return; fi
    case "$uchoice" in
      1)
        convert_strategies ;;
      2)
...
settings_menu() {
  while true; do
    ...
    echo "  0) Back"
    echo ""
    if ! read -rp "Выберите опцию: " sch; then return; fi
    case "$sch" in
      1) toggle_autorun ;;
      2) toggle_gamefilter ;;
      3) toggle_ipset ;;
...
check_for_updates() {
  ...
  if [ ${#conflicted[@]} -gt 0 ]; then
    ...
    if ! read -rp "Ваш выбор [1/2/3]: " uchoice; then return; fi
    case "$uchoice" in
      1)
...
updates_menu() {
  while true; do
    ...
    echo "  0) Back"
    echo ""
    if ! read -rp "Выберите опцию: " ucho; then return; fi
    case "$ucho" in
      1) check_for_updates ;;
...
MENU
  if ! read -rp "Ваш выбор: " choice; then exit 0; fi
  case "$choice" in
    1) manage_strategy ;;
    ...
    5) utilities_menu ;;
    9) delete_zapret ;;
    0) clear_screen; echo "Выход."; exit 0 ;;
    *) echo "Неверный выбор."; read -rp "Нажмите Enter...";;
  esac
}
...
install_selected_strategy() {
  while true; do
    ...
    if ! read -rp "Выберите стратегию (номер): " strat_choice; then
      echo ""
      return
    fi

    if [[ "$strat_choice" =~ ^[0-9]+$ ]] && [ "$strat_choice" -ge 0 ] && [ "$strat_choice" -le "${#strategies[@]}" ]; then
      break
    fi

    echo -e "\033[0;31mНеверный ввод. Пожалуйста, выберите номер из списка.\033[0m"
    sleep 1
  done
  ...
  while true; do
    ...
    if ! read -rp "Выберите (1 или 2): " autorun_choice; then
      echo ""
      return
    fi
    ...
    echo -e "\033[0;31mНеверный выбор. Введите 1 или 2.\033[0m"
    sleep 1
  done
