...
```
1. Сделайть файл **linux-strategies/convert-strategies.sh** исполняемым ( команда: chmod +x)
2. Запустить файл `service.sh`
3. Выполнить конвертацию стратегий, выбрав пункт **`Convert strategies`** в меню (нужно для первого запуска или после добавления новых стратегий)
4. Выбрать нужную стратегию в пункте **`Install Service`** в меню
